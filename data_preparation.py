...
def load_data(file_path: str) -> pd.DataFrame:
    """Load dataset from CSV and print initial shape."""
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    """Clean dataset by removing duplicates and invalid occupancy rows."""
    initial_rows = len(df)
...
def process_datetime(df: pd.DataFrame) -> pd.DataFrame:
    """Convert LastUpdated to datetime and sort ascending."""
...
def split_data(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Split data into train/validation/test sets without shuffling."""
    n = len(df)

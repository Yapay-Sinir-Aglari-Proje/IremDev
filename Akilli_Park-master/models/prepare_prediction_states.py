...
def load_predictions(input_path: Path) -> pd.DataFrame:
    ...
    except Exception as exc:  # pragma: no cover - savunmacı dosya okuma kontrolü
        raise ValueError(f"Tahmin dosyası okunamadı: {input_path}") from exc
...
def _normalize_empty_slots_within_timestamp(df: pd.DataFrame) -> pd.Series:
    ...
    # Eğer aynı zaman dilimindeki tüm otoparkların boş slot sayısı eşitse,
    # sıfıra bölme hatasını önlemek için availability yüksek (1.0) atanır.
...
def create_rl_features(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    # Doluluk değerini fiziksel olarak geçerli aralıkta tut:
    # 0 <= predicted_occupancy <= capacity
    ...
    # Gelecekte boş olacak slot sayısı RL ajanı için fırsat bilgisidir.
    df["predicted_empty_slots"] = df["capacity"] - df["predicted_occupancy"]

    # Doluluk oranı trafik yoğunluğunu özetleyen kompakt bir sinyaldir.
    df["occupancy_ratio"] = (df["predicted_occupancy"] / df["capacity"]).clip(0.0, 1.0)

    # Availability score aynı zaman anında otoparklar arası
    # karşılaştırmalı boşluk avantajını gösterir.
    df["availability_score"] = _normalize_empty_slots_within_timestamp(df)

    # Risk seviyeleri hem yorumlanabilir dashboard hem de
    # reward shaping için kullanılabilir.
    conditions = [
    ...
    df["risk_level"] = np.select(conditions, risk_levels, default="HIGH")

    # Saat bilgisini döngüsel encode ederek 23 -> 00 sürekliliği korunur.
    hour = df["timestamp"].dt.hour.astype(float)
    df["time_sin"] = np.sin(2 * np.pi * hour / 24.0)
    df["time_cos"] = np.cos(2 * np.pi * hour / 24.0)

    # İleride araç ile park alanı arası mesafe feature’ı eklenecek.
    df["distance_placeholder"] = 0.0

    # Reward fonksiyonunda congestion cost olarak kullanılabilecek
...
def build_state_vector(df: pd.DataFrame) -> pd.DataFrame:
    """Seçilen sayısal feature'lardan kompakt RL state vector oluşturur."""
    vector_columns = [
        "occupancy_ratio",
    ...
    # RL ajanına verilecek sayısal durum vektörü oluşturulur.
    df["rl_state_vector"] = df[vector_columns].apply(_to_vector, axis=1)
    return df
...
def save_rl_states(df: pd.DataFrame, output_path: Path) -> None:
    ...
    except Exception as exc:  # pragma: no cover - savunmacı dosya yazma kontrolü
        raise IOError(f"RL state dosyası kaydedilemedi: {output_path}") from exc
...
def main(
    ...
    try:
        ...
        LOGGER.exception("RL state hazırlama başarısız oldu: %s", exc)
        raise
